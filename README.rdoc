...
3. Remember, you have to "git add" changes to ready it for a commit,
  $ git add *.*

then "git commit" to commit those changes locally,
  $ git commit -m 'Add all the octocat txt files'

< if first time then >
  $ git remote add origin https://github.com/{ #name }.git

then "git push origin master" to push it to the remote "origin" repository,
  $ git push -u origin master

which is the previously created github.com repository.
...
6. You may need to run migrations on heroku afterwards, with
